	...
	public void useBonus(ArrayList<String> bonus, Player player) {
		...
		for (String singleBonus : bonus) {
			...
			case "VICTORYTRACK":
				...
				PubSub.notifyAllClients(this.players, "Player with nickname '" + player.getNickName() + "' won "
						+ numberOfBonus + " points for the Victory Track!");
				break;
			case "POLITIC":
			...
			case "NOBILITYTRACK":
				...
				PubSub.notifyAllClients(this.players, "Player with nickname '" + player.getNickName() + "' won "
						+ numberOfBonus + " bonus steps in Nobility Track!");
